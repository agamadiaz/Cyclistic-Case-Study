...
As part of Coursera’s ‘Google Data Analytics Certificate’ program,
completing a Case Study was the final portion of the program. This Case
Study was used to highlight what I learned from the program and showcase
my evolution in Data Analysis.
...
Cyclistic bike differently?* I will be using the publicly available
dataset from a real bike sharing company called Divvy. (Due to the
fictional nature of Cyclistic no real data is available)

Data set here: <https://divvy-tripdata.s3.amazonaws.com/index.html> (The
data has been made available by Motivate International Inc. under the
following license: <https://www.divvybikes.com/data-license-agreement>)

I will be using one year of data using the Q2\_2019, Q3\_2019, Q4\_2019,
and Q1\_2020 data. I will be using the program RStudio to perform my
data analysis including cleaning of data, analyzation, and basic
visualizations.

An assessment of the integrity of the data reveals two items of note. 1)
Due to data-privacy issues, riders’ personally identifiable information
is hidden. 2) This means I won’t be able to connect pass purchases to
credit card numbers to determine if casual riders are visiting the
service area or if they have purchased multiple single passes.

# Data Analysis Phase

To begin I will load previously installed packages that will help with
cleaning and consolidating data, change and read date attributes, and
visualize the data.
...
Now that the data has been loaded into the Script/Console I will do a
quick glimpse of the data to see what I am working with.
...
Right away I notice there are an uneven amount of columns and that there
are different column names for the same type of data. I will rename
columns to match one another using the newest data set as a guide in
order to combine the data sets and eliminate unused columns.
...
Now that the columns have been renamed I will check them all for further
inconsistencies.
...
Checking this time I notice that columns ‘ride\_id’ and ‘rideable\_type’
need to be changed to character in order to stack. To do this I run the
following:
...
Now I check the column names again to make sure the data matches and
will be able to merge into one data set.
...
Now, I notice ‘end\_station\_id’ and "start\_station\_id’ need to go
...
Seeing that indeed the columns will stack and therefore the data will be
consolidated; it is time to merge the four data sets into one.

``` r
all_trips <- bind_rows(q2_2019,q3_2019,q4_2019,q1_2020)
```

Excellent, the four data sets are now combined into one set named
‘all\_trips’ It is now time to remove the unused columns.
...
Four columns of data were removed. Now I will check the remaining
columns, generate a table of the new data set, and generate a summary in
...
I am down to nine columns of data, and looking through the table that
was generated, spot some changes that need to be implemented.

The first change is to combined the four different names used for riders
into two usable names. I will check the data before and after to make
sure the merger worked.
...
Now that I verified that the changes worked, it is time for the next
change. I aim to split up the Dates into a more usable format by having
separate columns for Month, Day, and Year of each ride.
...
Now I can use the different columns to combine or analyze the data if
needed.
...
I can see that the new ‘ride\_length’ is in Factor format and needs to
be in Numeric format to be able to run calculations on the data.
...
At this point I generate a table again in order to play around with the
data and see what else needs to be cleaned, changed, or added.

``` r
View(all_trips)
```

Everything is looking much better and now it is time to clean the “bad”
data. There are negative ride\_length entries as well as entries that
appear to be taken to be serviced. A new data set will be created due to
...
The summary of the new data set ‘all\_trips\_2’ shows me the min,
median, mean, and max points of data.
...
Looking at the aggregate data of ‘ride\_length’ I can start to see that
there is a large difference between the average ride length of causal
users vs members.

Now I will check the average ride time per day of the week for both
types of members.
...
Noticing that the dates are out of order, I run the following to adjust
this:
...
This results in a tibble (data frame) showing member amount of rides and
duration per weekday.

At this point it is time to revisit the original Business Task and see
if there is progress towards the question that needs to be answered.

How do annual members and casual riders use Cyclistic bikes differently?

To answer this question I will run two data visualizations that can shed
some key differences in riders.
...
![](Testing_files/figure-gfm/unnamed-chunk-23-1.png)<!-- -->

After some trial and error I was able to make the necessary changes to
provide a clean visual. I am grouping by ‘member\_casual’ or member type
and weekday, using a formula of ‘number\_of\_rides’ (divided by ten
thousand in order to fit the data better into the visual) and using the
mean of ‘ride\_length’ as ‘average\_duration’. Next, it is arranged by
member type and weekday. The actual type of visual and its elements are
next, setting the X and Y axis using the ‘weekday’, and
‘number\_of\_rides’ data. The fill or what the column will show is the
member type. A column/bar chart was chosen due to the data that was
being compared, one type versus one type. Lastly, I labeled the visual
to be able to read it at a glance.
...
![](Testing_files/figure-gfm/unnamed-chunk-24-1.png)<!-- -->

Almost identical set up to the first visual. The key changes are using
‘average\_duration’ as the Y axis, changing the labels, and dividing
the ‘ride\_length’ mean by 60 as the duration was in seconds thus
dividing by 60 yields minutes.

## Conclusions

The purpose of this Case Study was to showcase my ability to follow the
data analysis process as detailed in Courseras ‘Google Data Analytics
Certificate’ program.

The ASK portion was the question that I intended to answer: How do
annual members and casual riders use Cyclistic bikes differently?

The PREPARE portion was to check where the data would come from. What
issues might arise. How I would go about analyzing the data.

The PROCESS phase was get a first feel for it and choose which program I
would use to analyze the data. Start cleaning the data, finding trends,
errors, changes that needed to be made and more.

The ANALYZE phase was to take the new data set I had created by merging
four different sets, cleaning it, and creating two visualizations to
explain my analysis from the data.

The SHARE phase, involves showing the visualizations that were made and
creating this R Markdown to document my journey through this Case Study.

Finally the ACT portion is to submit this Case Study for the program,
and share this Case Study in my portfolio as a testament to the start of
